...
use Awz\Utm\Access\Custom\ComponentConfig;
                ...
                window.awz_right_dialog = {
                    ...
                    reloadItems: function(items, event){
                        ...
                        items.forEach(function(itm){
                            if(itm.getEntityId()==='user' || itm.getEntityType()==='awzutm-user'){
                                let itmEv = {
                                    type: 'users',
                                    id: 'U'+itm.getId(),
                                    key: 'U'+itm.getId(),
                            ...
                            }else if(itm.getEntityType()==='awzutm-group'){
                                let itmEv = {
                                    type: 'groups',
                                    id: 'G'+itm.getId(),
                                    key: 'G'+itm.getId(),
                    ...
                    load: function(groupKey, options){
                        ...
                        if(groupData.hasOwnProperty('members')){
                            for(k in groupData['members']){
                                ...
                                if(itm.type==='users'){
                                    preselectedItems.push(['awzutm-user',itm.id]);
                                }
                                else if(itm.type==='awzutm-user'){
                                    preselectedItems.push(['awzutm-user',itm.id]);
                                }
                                else if(itm.type==='awzutm-group'){
                                    preselectedItems.push(['awzutm-group',itm.id]);
                                }
